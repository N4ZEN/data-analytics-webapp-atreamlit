...
def home_page(clean_df, final_df, cluster_insights_df, cluster_names_dict):
    ...
    if st.sidebar.checkbox('Upload new data', key='new_upload_toggle'):
        ...
        update_data = st.file_uploader("Choose a CSV file", type="csv", key="update_dataset")
        if update_data is not None:
            new_data = pd.read_csv(update_data,  encoding='ISO-8859-1')
...
if __name__ == "__main__":
    ...
    if check_dataset() == False:
        ...
        if path.exists(default_dataset):
            ...
            uploaded_file = st.file_uploader("Choose a CSV file", type="csv", key="create_dataset")
